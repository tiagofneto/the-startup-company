...
import { getWallet, transfer_tokens_to_address, transfer_tokens_to_handle } from '../src/aztec.js';
...
if (args.includes('--handle')) {
  ...
  await transfer_tokens_to_handle(wallet, companyRegistry, from, to, 1);
} else if (args.includes('--address')) {
  const to = args[args.indexOf('--address') + 1];
  await transfer_tokens_to_address(wallet, companyRegistry, from, AztecAddress.fromString(to), 1);
} else {
